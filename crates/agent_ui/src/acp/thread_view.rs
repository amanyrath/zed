            ...
            this.update_in(cx, |this, window, cx| {
                match result {
                    Ok(thread) => {
                        ...
                        if let Some(native_connection) = thread.read(cx)
                            .connection()
                            .clone()
                            .downcast::<NativeAgentConnection>()
                        {
                            let activity_tracker = native_connection.activity_tracker(cx);
                            let client = this.workspace.upgrade().and_then(|workspace| {
                                workspace.read(cx).app_state().client.clone().into()
                            });
                            
                            if let Some(client) = client {
                                // Send initial state if agent is already active and we're in a room
                                if let Some(room) = ActiveCall::try_global(cx)
                                    .and_then(|call| call.read(cx).room().cloned())
                                {
                                    if let Some(user_id) = client.user_id() {
                                        let tracker = activity_tracker.read(cx);
